"use server";

import { builderApiUrl } from "@/utils/config";

interface WebsiteContent {
  html: string;
  markdown: string;
  title: string;
  description: string | null;
  url: string;
  screenshot: string;
  images: Array<{
    url: string;
    alt: string;
    isLogo: boolean;
  }>;
  designMetadata?: {
    colors: string[];
    fonts: string[];
    hasAnimations: boolean;
    animationLibrary: string | null;
  } | null;
  isProtected?: boolean;
}

interface CloneWebsiteResult {
  success: boolean;
  data?: WebsiteContent;
  method?: "remote" | "url-only";
  error?: string;
}

const SCRAPER_ENDPOINT = `${builderApiUrl}/scrape-simple`;

const emptyWebsiteData = (url: string): WebsiteContent => ({
  html: "",
  markdown: "",
  title: "",
  description: null,
  url: url,
  screenshot: "",
  images: [],
  designMetadata: null,
});

export async function cloneWebsite(url: string): Promise<CloneWebsiteResult> {
  try {
    if (!url) {
      throw new Error("URL is required");
    }

    console.log(`🚀 Starting website clone: ${url}`);

    if (!SCRAPER_ENDPOINT) {
      throw new Error(
        "Scraper endpoint is not configured. Please set WEBSITE_SCRAPER_URL or BUILDER_URL.",
      );
    }

    const response = await fetch(SCRAPER_ENDPOINT, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      cache: "no-store",
      body: JSON.stringify({ url }),
    });

    if (!response.ok) {
      throw new Error(
        `Remote scraper responded with status ${response.status}`,
      );
    }

    const payload = await response.json();

    if (payload?.success === false) {
      throw new Error(payload?.error || "Remote scraper failed");
    }

    const data = payload?.data ?? payload;

    if (!data || typeof data !== "object") {
      throw new Error("Remote scraper returned an invalid payload");
    }

    const websiteData: WebsiteContent = {
      html: data.html ?? "",
      markdown: data.markdown ?? "",
      title: data.title ?? "",
      description: data.description ?? null,
      url: data.url ?? url,
      screenshot: data.screenshot ?? "",
      images: Array.isArray(data.images) ? data.images : [],
      designMetadata: data.designMetadata ?? null,
      isProtected: data.isProtected ?? false,
    };

    if (websiteData.isProtected) {
      console.log(
        "⚠️ Site is protected, continuing with URL only (no error to user)",
      );
      return {
        success: true,
        data: emptyWebsiteData(url),
        method: "url-only",
      };
    }

    console.log("✅ Scraping successful:", {
      title: websiteData.title,
      hasMarkdown: !!websiteData.markdown,
      hasScreenshot: !!websiteData.screenshot,
      imagesFound: websiteData.images?.length || 0,
      hasDesignMetadata: !!websiteData.designMetadata,
      animationLibrary: websiteData.designMetadata?.animationLibrary || null,
    });

    return {
      success: true,
      data: websiteData,
      method: "remote",
    };
  } catch (error) {
    console.error("❌ Error in cloneWebsite:", error);

    const errorMessage =
      error instanceof Error ? error.message : "Unknown error";

    if (
      errorMessage.startsWith("TIMEOUT:") ||
      errorMessage.includes("timeout") ||
      errorMessage.includes("TIMEOUT:") ||
      errorMessage.startsWith("DNS_ERROR:") ||
      errorMessage.startsWith("ACCESS_FORBIDDEN:") ||
      errorMessage.includes("ENOTFOUND") ||
      errorMessage.includes("DNS") ||
      errorMessage.includes("403") ||
      errorMessage.includes("Forbidden")
    ) {
      console.log(
        "⚠️ Site access issue, continuing with URL only (no error to user)",
      );
      return {
        success: true,
        data: emptyWebsiteData(url),
        method: "url-only",
      };
    }

    return {
      success: false,
      error: errorMessage,
    };
  }
}
